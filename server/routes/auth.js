...
// Route 2: Login User: POST: http://localhost:8181/api/auth/login. No Login Required
router.post('/login', [
  body('email', "Please Enter a Valid Email").isEmail(),
], passport.authenticate('local', {
  failureRedirect: '/login-failure', // Optional
  failureFlash: true, // Optional, if you want to use flash messages
}), (req, res) => {
  res.status(200).json({ authtoken: req.user.authtoken });
});

// Route 3: Update User Information: PUT: http://localhost:8181/api/auth/update. Authentication Required
router.put('/update', [
...
// Route 4: Fetch user data based on the email: GET: http://localhost:8181/api/auth/user. Authentication Required
...
// Route 5: Update user information: PUT: http://localhost:8181/api/auth/user. Authentication Required
router.put('/user', [
  body('name', "Username should be at least 4 characters").isLength({ min: 4 }),
  body('phone', "Phone number should be 10 digits").isLength({ min: 10 }),
], async (req, res) => {
